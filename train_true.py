...
def create_model(num_joints, load_pretrain_weights=False):
    ...
    if load_pretrain_weights:
        ...
        weights_dict = torch.load("/home/maihaoxiang/other_person/Fetal-head-segmentation-and-circumference-measurement-from-ultrasound-images-master/save_weights/model-209.pth", map_location={'0':'gpu'})
...
def main(args):
    ...
    train = Data('train', transforms=data_transform["train"])
    val = Data('validate', transforms=data_transform["train"])
    # 注意这里的collate_fn是自定义的，因为读取的数据包括image和targets，不能直接使用默认的方法合成batch
    batch_size = args.batch_size
    nw = min([os.cpu_count(), batch_size if batch_size > 1 else 0, 8])  # number of workers
    print('Using %g dataloader workers' % nw)
    
    train_data_loader = data.DataLoader(train,
    ...
    val_data_loader = data.DataLoader(val,
                                      batch_size=batch_size,
                                      shuffle=False,
                                      pin_memory=True,
                                      num_workers=nw,
...
if __name__ == "__main__":
    ...
    # 训练数据集的根目录(coco2017)
    #parser.add_argument('--data-path', default='C:/NUS/project5003/dataset/coco2017', help='dataset')
    # COCO数据集人体关键点信息
    #parser.add_argument('--keypoints-path', default="C:/NUS/project5003/HRNet/HRNet/person_keypoints.json", type=str,
