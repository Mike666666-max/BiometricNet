...
class KeypointToHeatMap(object):
    ...
    def __call__(self, image, target):
        kps = target["keypoints"].astype(np.int) 
        num_kps = kps.shape[0]
        ...
        for kp_id in range(num_kps):
            ...
            # 计算高斯核有效区域（高斯核坐标系）
            g_x = (max(0, -ul[0]), int(min(br[0], self.heatmap_hw[1] - 1) - ul[0]))
            g_y = (max(0, -ul[1]), int(min(br[1], self.heatmap_hw[0] - 1) - ul[1]))
            # image range
            ...
            if kps_weights[kp_id] > 0.5:
                ...
                # imgy = img_y.tolist()
                # imgx = img_x.tolist()
                heatmap[kp_id][int(img_y[0]):int(img_y[1]) + 1, int(img_x[0]):int(img_x[1]) + 1] =\
                    self.kernel[int(g_y[0]):int(g_y[1]) + 1, int(g_x[0]):int(g_x[1]) + 1]
